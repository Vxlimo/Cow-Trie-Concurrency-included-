...
void concurrentSearch(sjtu::TrieStore& trie, int start, int end, bool button)
{
    for (int i = start; i < end; ++i) {
        ...
        if (button && (!result.has_value() || (*(result.value()) != ("value" + std::to_string(i))))) {
            if (!result.has_value()) {
                std::cerr << "Failed search for key: "
                          << "key" + std::to_string(i) << std::endl;
            } else {
                std::cerr << "Failed search for key: "
                          << "key" + std::to_string(i) << " with value: " << *(result.value()) << std::endl;
            }
            failedSearches++;
        }
    }
...
void testConcurrency()
{
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (trie.Get<std::string>("key" + std::to_string(i)).has_value()) {
            std::cerr << "Consistency test failed for key: "
                      << "key" + std::to_string(i) << std::endl;
        }
    }
